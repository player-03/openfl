...
class GLTilemap {
	...
	public static function render (tilemap:Tilemap, renderSession:RenderSession):Void {
		...
		gl.bindTexture (gl.TEXTURE_2D, tilemap.tileset.getTexture (gl));
		
		var tiles = tilemap.__tiles;
		var count = tiles.length;
		var uvs = tilemap.__uvs;
		...
		var offset, tile, tileMatrix, x, y, x2, y2, x3, y3, x4, y4;
		...
		for (i in 0...count) {
			...
			if (tile.id != cacheTileID) {
				...
				tileWidth = Std.int (tilemap.__rects[tile.id].width);
				tileHeight = Std.int (tilemap.__rects[tile.id].height);
				cacheTileID = tile.id;
